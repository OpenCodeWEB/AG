/**
 * GitHub webhook event handler — full processing pipeline.
 *
...
/**
 * Handle push events — the main processing pipeline.
        ...
        if (fixedContents.length > 0) {
          ...
          try {
            ...
            const prUrl = await createPullRequest(
              ...
              `robot: OpenCodeWEBsAG auto-repair [skip ci]`,
              `🤖 **OpenCodeWEBsAG Auto-Repair Report**\n\n${allFixes.map(f =>
                `- ✅ ${f.fixApplied}`).join("\n")}\n\n---\n\n_Auto-generated by OpenCodeWEBsAG_`,
            );
          ...
          // Issues found but not auto-fixable — log only (future: create issues)
...
/**
 * Handle pull_request events — post a review comment with scan results.
 */
  ...
  ctx.waitUntil(
    (async () => {
      try {
        ...
        let comment = `## 🤖 OpenCodeWEBsAG Code Review\n\n`;
...
/**
 * Handle installation events — sync KV when app is installed or uninstalled.
 */
...
async function handleIssueComment(
  ...
  // Placeholder — future: respond to specific commands (e.g., "/scan")
